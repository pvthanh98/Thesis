...
const MyMapComponent = compose(
  ...
  >
    ...
    {props.rescueLocation &&
      props.rescueLocation.map((rescue) => (
        <Marker
          key={rescue._id}
          position={{
...
export default function Dashboard() {
  ...
  const loadChartData = () => {
    axios()
      .get("/api/bill/chart/count/week")
      .then((resl) => setChartCountWeek(resl.data))
      .catch((err) => console.log(err));
    // const
    axios()
      .get("/api/bill/chart/cost/week")
        ...
        <GridItem xs={12} sm={6} md={3}>
          <Card>
            <CardHeader color="warning" stats icon>
              ...
              <h3 className={classes.cardTitle}>{customerToday}</h3>
            </CardHeader>
            <CardFooter stats>
