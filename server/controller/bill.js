    ...
    try {
      ...
      if (!req.body.coordinate) {
        ...
        data.coordinate = {
		  ...
		  // dùng coordinate từ rescue gửi lên
        data.coordinate = {
          lat: req.body.coordinate.lat,
          lng: req.body.coordinate.lng,
        };
        console.log("get coordinate from rescue");
      }
      const geocoderResp = await axios.get(
        `https://maps.googleapis.com/maps/api/geocode/json?latlng=${data.coordinate.lat},${data.coordinate.lng}&key=AIzaSyAB5wWf_sSXn5sO1KE8JqDPWW4XZ8QKYSQ`
      );
      data.address = geocoderResp.data.results[0].formatted_address;
      Bill.create(data)
        .then(() => {
		  if(req.body.rescue_id){
			  Rescue.findByIdAndUpdate(req.body.rescue_id, {is_complete : true})
			  .then(()=>console.log("update rescue"))
